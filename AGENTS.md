...
- Write like you talk (`research/human-voice.md` is the full playbook). Don't let a sentence through unless it's the way you'd say it to a smart friend. Contractions by default in English. Ration the signature devices: "It is not X. It is Y." at most once per post, aphorism closers at most one per section, one unanswered rhetorical question per post. Vary paragraph length on purpose — uniform rhythm reads as machine writing. Don't reuse an opening or closing frame from recent posts. 中文版从论点重写而不是从英文句子翻译：拆欧化句式（万能动词、"在……的情况下"、前置长定语、多余的"被"），多用句号，口语词不硬加，不排比，不升华。
