...
| decisiveness | 决断力 | The term the outside piece the "decisiveness isn't a skill" note argues with uses for its scarce thing. Keep 决断力 when reporting that argument; the note's own claim is that the word glues two things together, so don't use it as q-notes' own category. |
| accountability (as the scarce thing) | 责任主体 | The half of "decisiveness" a model can't supply: whose name and comp move when the call goes wrong. 可追责 works as the adjective form; 责任主体 for the noun. Not 问责制 (that's institutional discipline, a different idea). |
| AI-native (game) | AI 原生（游戏） | Coined in the "AI-native game is a test" note for the removal-test definition (remove the model, does the core loop collapse); keep "AI" in English, 原生 for "native". Contrast: AI-augmented content, where removing the model leaves the game intact. |
| embodied intelligence | 具身智能 | Used in the "embodied intelligence is the intelligence, not the body" note for the perception-decision-action-feedback loop coupling a body to its environment; keep both terms untranslated as-is on first mention where the piece is directly discussing the Chinese-market term (e.g. product marketing), English "embodied intelligence" otherwise. Distinct from the removal test coined for AI-native games — this piece's version asks whether the *decision* step is intelligent, not merely whether a feedback loop exists in the engineering sense. |
