...
- 2026-08-05 (A/B choice, PR #58) Never（中文）: 破折号能用逗号就用逗号。作者选了 zh 结尾的 B 之后专门提出来："can you please make sure to avoid using dash here it could simply be replaced by a comma"。破折号做戏剧性停顿是中文里最重的机器味之一，整篇适用，不只是被引的那一句。
- 2026-08-05 (A/B choice, PR #58) Move: 开头要落在一个具体的对象上，别用"X 这个词被到处乱用"这类泛化设定——三个选项里作者挑了唯一点名提问的那个（"here's the question nobody's answering: what actually makes a game 'AI-native'?"），并且评价是"A and B are too generic. C is only slightly better"：连被选中的那个都还不够具体。
- 2026-08-05 (A/B choice, PR #58) Move: 结尾不要把整篇收窄到一个技术取舍上。作者对被选中的 en 结尾的评语："that already seems very narrow because AI native games can actually mean so many different things far more than game assets generation." 机制层面的收束要给，但要让读者看见这个话题比被讨论的那一维大。
