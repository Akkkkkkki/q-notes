...
So here's where I land, for now. "AI-native" is a real, checkable category: does the loop survive without the model, yes or no. Whether an AI-native game is worth playing is a separate, harder question, and today's answer is mostly no, because taste isn't something a system optimizing for the expected case can produce on its own. Making the model bigger won't fix that. Pointing runtime generation at one specific choice instead of the safe average might. Design has to solve it.
