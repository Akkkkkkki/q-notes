...
Software does have an equivalent consequence gate, just not where the DAC story looks. Aviation software has to clear [DO-178C](https://www.faa.gov/documentLibrary/media/Advisory_Circular/AC_20-115D.pdf), and the FAA's compliance findings come from a Designated Engineering Representative, an engineer it personally authorizes to sign on its behalf, or from an organization it has delegated that same authority to. Either way, the authorization is revocable. Strip the legal machinery away and it's the same mechanism as Calibre and Questa One, wearing a regulatory costume instead of a technical one.
